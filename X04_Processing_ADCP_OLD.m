...
for i1=1:length(File)
    ...
    t_VMP=[t_VMP;ti];
    clc;
    clearvars -except t_VMP P_VMP File data_dir
end
% ADCP 300 kz
...
while 1
    ...
    if m(count)~=m(count-1)
        ...
        U300(count2,:)=nanmean(DataU300(ind1:ind2,7:end));
        V300(count2,:)=nanmean(DataV300(ind1:ind2,7:end));
        %%%% haz las operaciones entre esto
        ind1=ind2+1;
        count2=count2+1;
    end
...
while 1
    ...
    if m(count)~=m(count-1)
        ...
        U600(count2,:)=nanmean(DataU600(ind1:ind2,7:end));
        V600(count2,:)=nanmean(DataV600(ind1:ind2,7:end));
        %%%% haz las operaciones entre esto
        ind1=ind2+1;
        count2=count2+1;
    end
...
Index=nan(1,8);
while 1
    m(count)=DataU1200(count,5);
    if m(count)~=m(count-1)
        ind2=count-1;
        %%%% haz las operaciones entre esto
        Index(count2,1:ind2-ind1+1)=ind1:ind2;
        t_1200(count2,1)=datetime(DataU1200(ind1,1),DataU1200(ind1,2),DataU1200(ind1,3),DataU1200(ind1,4),DataU1200(ind1,5),0); 
        U1200(count2,:)=nanmean(DataU1200(ind1:ind2,7:end));
        V1200(count2,:)=nanmean(DataV1200(ind1:ind2,7:end));
        %%%% haz las operaciones entre esto
        ind1=ind2+1;
        count2=count2+1;
    end
    count=count+1;
    if count>size(DataU1200,1)
        break
    end
end
Index(Index==0)=NaN;    
...
U600=U600(74:1154,9:end)';
V600=V600(74:1154,9:end)';
t_600=t_600(74:1154,:);

U1200=U1200(75:1155,5:end)';
V1200=V1200(75:1155,5:end)';
t_1200=t_1200(75:1155,:);
...
DataU2021=movmean(DataU2021,61,2,'omitnan');% En time
DataU2021_filt=movmean(DataU2021,5,1,'omitnan');% Vertical
DataV2021=movmean(DataV2021,61,2,'omitnan');
DataV2021_filt=movmean(DataV2021,5,1,'omitnan');
idU=isnan(DataU2021);
...
clearvars -except *2021 data_dir *filt
...
for i1=1:length(File)
    ...
    t_VMP=[t_VMP;ti];
    clc;
    clearvars -except t_VMP P_VMP File data_dir *2021 *filt
end
...
DataU2022_filt=movmean(DataU2022,3,1,'omitnan');% Vertical
DataV2022=movmean(DataV2022,31,2,'omitnan');
DataV2022_filt=movmean(DataV2022,3,1,'omitnan');
idU=isnan(DataU2022);
...
clearvars -except t_VMP File *2021 *2022 data_dir *filt
