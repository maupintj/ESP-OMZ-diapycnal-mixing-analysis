function dgts_tesis_O(SA,CT,O,p_ref,isopycs,limits,title_string)
...
hold on;
for i1=1:size(SA,2)
    H=scatter(SA(:,i1),CT(:,i1),50,O(:,i1),'filled');
end
cmap=cmocean('oxy');
colormap(cmap);
caxis([0 250])
% c10 = colorbar;
% c10.Label.String = 'Oxygen [{\mu} M]';
% c10.Limits=[0 250];
% %set(c2,'Marker','o','MarkerSize',5*sz,'MarkerEdgeColor','none', ...
%  % 'MarkerFaceColor','b');

if exist('c1','var')
    clabel(c1,h,'labelspacing',360,'fontsize',10*sz,'color',[.5 .5 .5]);
end
...
if exist('title_string','var')
    title([title_string],'fontsize',9*sz);
else
