...
if we_have_S
    if we_have_JAC_T 
        ...
        [S_CTD, C_CTD_match,~] = salinity_JAC (P_slow, T_CTD, C_CTD,...
                          'fs',fs_slow,'speed',mean_speed); %modificada Mauro
    else 
...
if ~isempty(P_start) && ~isempty(P_end) % will actually be t_start and t_end for HMP
    ...
    if isempty(range)
        if ~isHMP
            ...
            ppmin=max([P_start,min(Indep_var_fast)]);
            ppmax=min([P_end,max(Indep_var_fast)]);
            range = find((Indep_var_fast > ppmin) & (Indep_var_fast <=    ppmax));
            disp(['Using instead range:' num2str(ppmin) ' to ' num2str(ppmax)])
%             if isempty (range)
%             warning(['The pressure range of ' num2str(P_start) ' to ' ...
%                 num2str(P_end) ' was not found in this profile'])
%                 range = find((Indep_var_fast > min(Indep_var_fast)) & (Indep_var_fast <= max(Indep_var_fast)))
%             end
        else
...
if ~isempty(range) && make_figures && plot_spectra
    ...
    %fig_aspectratio(gcf,2);
    sp(1) = axes('Position',[0.09 0.09 0.87 .78]); % wavenumber spectra
...
diss.SH_BP = SH_BP; % Añadida por Mauro
diss.SH_HP = SH_HP; % Añadida por Mauro
diss.P_fast=Indep_var_fast; %Añadida por Mauro
% - Set return value
result = diss;
